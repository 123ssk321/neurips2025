...
class LinkPGExplainer(ExplainerAlgorithm):
    ...
    def train(
        ...
        inputs = self._get_inputs(z, edge_index, edge_label_index)
        ...
        if self.model_config.task_level in (ModelTaskLevel.node, ModelTaskLevel.edge):
            _, hard_edge_mask = self._get_hard_masks(model, edge_label_index.view(2), edge_index,
                                                     num_nodes=x.size(0))
            edge_mask = edge_mask[hard_edge_mask]

        y_hat, y = model(x, edge_index, **kwargs), target
        if index is not None and self.model_config.task_level == ModelTaskLevel.node:
    ...
    def _get_inputs(self, embedding: Tensor, edge_index: Tensor,
        ...
        if self.model_config.task_level == ModelTaskLevel.edge:
            assert index is not None
            zs.append(embedding[index[0, 0]].view(1, -1).repeat(zs[0].size(0), 1))
            zs.append(embedding[index[1, 0]].view(1, -1).repeat(zs[0].size(0), 1))
